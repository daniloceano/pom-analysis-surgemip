...
def _get_raw_bytes(
    ...
    if stations_dir is not None:
        ...
        # GESLA-4 extracted files may have quality suffixes (e.g. _rq, _fd).
        # Use _find_stem_in_namelist logic on the directory listing.
        dir_entries = [p.name for p in stations_dir.iterdir()] if stations_dir.exists() else []
        match = _find_stem_in_namelist(file_name, dir_entries)
        if match:
            return (stations_dir / match).read_bytes()
        logger.debug("  File not found at %s (tried with suffixes too)", stations_dir / file_name)
        return None
